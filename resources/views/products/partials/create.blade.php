<x-slide-form button-icon="plus" title="Add New Product">
    ...
    <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data" class="mx-8">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4  ">
            ...
            <div>
                ...
                <select name="type" id="type" required class="input @error('type') border-red-500 @enderror">
                    <option value="product" {{ old('type')=='product' ? 'selected' : '' }}>Product (Inventory tracked)
                    </option>
                    <option value="service" {{ old('type')=='service' ? 'selected' : '' }}>Service (No inventory)
                    </option>
                </select>
                @error('type')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            {{-- 4. Purchase Price --}}
            <div>
                <label for="purchase_price" class="label">Purchase Price ($) <span class="text-red-600">
                        *</span></label>
            ...
            <div>
                ...
                <input type="number" min="0" name="quantity_alert" id="quantity_alert"
                    value="{{ old('quantity_alert', 10) }}" required
